...
export default function Home() {
  ...
  const sendAnswers = async () => {
    ...
    try {
      const response = await fetch("https://formsubmit.co/ajax/shyherman99630@hotmail.com", {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({
          _subject: "💌 New date reservation from herman99.click",
          "Date / 日期": selectedDate,
          "Exact time / 具体时间": `${hour}:${minute} ${period}`,
          "Other available times / 其他时间": times.map(value => `${value} / ${timeZh[value]}`).join(" | "),
          "Activities / 约会活动": selectedActivities.map(value => `${value} / ${activityZh[value]}`).join(" | "),
          "Food / 想吃的东西": selectedFoods.map(value => `${value} / ${foodZh[value]}`).join(" | "),
          "Page language / 页面语言": language === "zh" ? "中文" : "English",
          "Submitted at / 提交时间": new Date().toLocaleString(language === "zh" ? "zh-CN" : "en-US"),
        }),
      });
    ...
    <main className="app-shell">
      ...
      <section className="invitation-card" aria-labelledby="page-title">
        ...
        {step === 6 && <div className="summary-content">
          ...
          <button className="start-over" onClick={()=>{ setSendStatus("idle"); setStep(0); }}>{t.restart}</button>
        </div>}
