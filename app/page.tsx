...
export default function CreatorPage() {
  ...
  return <main className="app-shell creator-shell"><div className="floating-heart heart-one">♥</div><div className="floating-heart heart-two">♥</div>
    <section className="invitation-card creator-card">
      ...
      {stage === "cover" && <div className="creator-cover"><div className="creator-cover-copy"><span className="eyebrow"><Sparkles className="h-4 w-4"/> {t.coverEyebrow}</span><h1>{t.coverTitle}</h1><p className="cover-bridge">{t.coverBridge}</p><p className="subtitle">{t.coverBody}</p><Button className="primary-button cover-cta" size="lg" onClick={()=>setStage("details")}>{t.coverCta} <ArrowRight className="h-4 w-4"/></Button><p className="cover-safe-note">✓ {t.noAccount}</p></div><div className="cover-visual"><span className="love-orbit">♥</span><Image src="/hello-kitty-couple.webp" alt="Hello Kitty and Dear Daniel holding a red heart" width={410} height={410} priority/></div><div className="cover-steps"><div><strong>①</strong><span>{t.howOne}</span></div><div><strong>②</strong><span>{t.howTwo}</span></div><div><strong>③</strong><span>{t.howThree}</span></div></div></div>}
